...
rec {
  ...
  # Generated using python3 (must use latin-1: python3's default open()
  # encoding is UTF-8, which encodes codepoints 128-255 as 2 bytes each,
  # breaking the 1-byte-per-value lookup below):
  # print(''.join([ chr(n) for n in range(1, 256) ]), file=open('ascii', 'w', encoding='latin-1'))
